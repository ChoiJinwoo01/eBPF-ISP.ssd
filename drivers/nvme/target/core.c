...
static u8 my_recv_opc;
static int my_recv_payload_len;
static char my_recv_payload[PAGE_SIZE + 1];
static u8 my_send_rdy;
static char my_send_payload[PAGE_SIZE + 1];

struct nvmet_ebpf ebpf;
EXPORT_SYMBOL_GPL(ebpf);


void nvmet_isc_recv(u8 opc, void *payload, int len)
{
	my_recv_opc = opc;
	my_recv_payload_len = len;
	if (len)
		strncpy(my_recv_payload, payload, PAGE_SIZE);
	sysfs_notify(my_kobj, NULL, "recv_rdy");
}

void nvmet_isc_send(u8 opc, void *payload, int *status)
{
	if (!my_send_rdy) {
		*status = 1;
		return;
	...
	if (payload)
		memcpy(payload, my_send_payload, PAGE_SIZE);
	*status = 0;
	my_send_rdy = 0;
}
...
static struct attribute isc_recv_rdy_attr = {
	.name = "recv_rdy",
	.mode = 0444,
};

static struct attribute isc_recv_payload_attr = {
	.name = "recv_payload",
	.mode = 0444,
};

static struct attribute isc_send_rdy_attr = {
	.name = "send_rdy",
	.mode = 0222,
};

static struct attribute isc_send_payload_attr = {
	.name = "send_payload",
	.mode = 0222,
};
...
static struct attribute *my_attrs[] = {
	&isc_recv_rdy_attr,
	&isc_recv_payload_attr,
	&isc_send_rdy_attr,
	&isc_send_payload_attr,
	NULL
};
...
{
	pr_info("my_show\n");
	if (strcmp(attr->name, "recv_rdy") == 0) {
		return sprintf(buf, "%u\n", my_recv_opc);
	} else if (strcmp(attr->name, "recv_payload") == 0) {
		memcpy(buf, my_recv_payload, my_recv_payload_len);
		return my_recv_payload_len;
	}
	return 0;
}
...
{
	pr_info("my_store\n");
	if (strcmp(attr->name, "send_rdy") == 0) {
		my_send_rdy = 1;
		return 1;
	} else if (strcmp(attr->name, "send_payload") == 0) {
		if (len > PAGE_SIZE)
			len = PAGE_SIZE;
		memcpy(my_send_payload, buf, len);
		return len;
	}
	return 0;
}

static struct sysfs_ops my_sops = {
	.show = my_show,
	.store = my_store,
};
...
static int __init nvmet_init(void)
{
	...
	error = kobject_add(my_kobj, NULL, "nvme_isc");
	if (error) {
		kobject_put(my_kobj);
		goto out;
	}
        ...
        for(i=0;i<16;i++){
                ebpf.map_page[i] = alloc_page(GFP_KERNEL);
                memset(page_address(ebpf.map_page[i]), 0, 4096);
        }

	pr_info("my nvmet init done\n");
...
static void __exit nvmet_exit(void)
{
	int i;
        ...
        for(i=0;i<16;i++){
                free_page((unsigned long)page_address(ebpf.map_page[i]));
        }
