...
void nvmet_execute_my_test(struct nvmet_req *req)
{
	int ret = 0;
	int i;
	struct scatterlist *sg;
	struct nvme_command *cmd = req->cmd;
	pr_info("nvmet_execute_mytest\n");
	pr_info("  cmd opc %u\n", cmd->common.opcode);
	pr_info("  cmd flags %u\n", cmd->common.flags);
	...
	pr_info("  cmd cdw2 %u\n", cmd->common.cdw2[0]);
	pr_info("  cmd cdw2 %u\n", cmd->common.cdw2[1]);
	pr_info("  cmd meta %lld\n", cmd->common.metadata);
	pr_info("  cmd dptr %lld\n", cmd->common.dptr.prp1);
	...
	pr_info("  cmd rw len %d\n", cmd->rw.length);
	pr_info("  cmd slba %llu\n", cmd->rw.slba);
	pr_info("  cmd slba %llu\n", cmd->rw.slba);

	pr_info("  data len %ld\n", req->data_len);
	pr_info("  transfer len %ld\n", req->transfer_len);
	pr_info("  sg cnt %d\n", req->sg_cnt);

	/*
	for_each_sg(req->sg, sg, req->sg_cnt, i) {
		void *mem;
		pr_info("    %d: len%d off%d\n", i, sg->length, sg->offset);
		mem = kmap_atomic(sg_page(sg));
		kunmap_atomic(mem);
	}
	*/

	if (cmd->common.opcode & 1) {
		if (req->sg_cnt) {
			void *mem;
			sg = req->sg;
			mem = kmap_atomic(sg_page(sg));
			pr_info("    0: len%d off%d\n", sg->length, sg->offset);
			nvmet_isc_recv(cmd->common.opcode, mem, sg->length);
			kunmap_atomic(mem);
		} else {
			nvmet_isc_recv(cmd->common.opcode, NULL, 0);
		}
	} else {
		if (req->sg_cnt) {
			int status;
			...
			mem = kmap_atomic(sg_page(sg));
			pr_info("    0: len%d off%d\n", sg->length, sg->offset);
			nvmet_isc_send(cmd->common.opcode, mem, &status);
			kunmap_atomic(mem);
			ret = status;
		} else {
			pr_info("   ???\n");
...
u16 nvmet_parse_admin_cmd(struct nvmet_req *req)
{
	...
	case nvme_admin_keep_alive:
		...
		req->data_len = 0;
		return 0;
	case nvme_admin_my_test_begin ... nvme_admin_my_test_end:
		pr_info("parse nvme admin my test\n");
		req->execute = nvmet_execute_my_test;
		req->data_len = le32_to_cpu(req->cmd->common.cdw12);
		return 0;
	}
